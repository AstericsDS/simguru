  ...
  @foreach ($kampusList as $kampus)
    <div class="mb-8">
      ...
      >
        ...
        <svg
          class="w-5 h-5 ml-2 transition-transform duration-200 {{ $openKampus[$kampus["id"]] ?? false ? "rotate-90" : "" }}"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
          viewBox="0 0 24 24"
        ...
        @foreach ($kampus["gedung"] as $gedung)
          <div class="ml-8 mt-2">
            ...
            >
              ...
              <svg
                class="w-4 h-4 ml-2 transition-transform {{ $openGedung[$gedung["id"]] ?? false ? "rotate-90" : "" }}"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
                viewBox="0 0 24 24"
              ...
              @foreach ($gedung["lantai"] as $lantai)
                <div class="ml-8 mt-2">
                  ...
                  >
                    ...
                    <svg
                      class="w-4 h-4 ml-2 transition-transform {{ $openLantai[$lantai["id"]] ?? false ? "rotate-90" : "" }}"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2"
                      viewBox="0 0 24 24"
                  ...
                  @if ($openLantai[$lantai["id"]] ?? false)
                    <ul class="ml-8 mt-2 space-y-2">
                      @foreach ($lantai["ruangan"] as $ruangan)
                        {{-- Ruang --}}
                        <li
                          class="flex items-center bg-white border border-black rounded px-4 py-2"
                        >
                          <span class="flex-1">
                            {{ $ruangan["nama"] }}
                          </span>
                          <button
                            class="bg-teal-700 text-white px-6 py-1 rounded hover:bg-teal-800 text-sm"
                          >
                            Details
                          </button>
                        </li>
