<div class="text-black">
    ...
    <div class="mt-4">
        ...
        <div class="flex justify-between items-center gap-2">
            ...
            <div class="flex gap-3 items-center">
                ...
                {{-- Pending Toggle --}}
                <div x-data="{ state: false }" @click="state = !state; $dispatch('pending-toggle', state)"
                    :class="state == true ? 'bg-yellow-300 border-yellow-300 text-white' : 'border-gray-300 text-gray-500'"
                    class="border py-2 px-3 rounded-md hover:bg-yellow-300 active:bg-yellow-400 hover:border-yellow-300 transition-all hover:text-white cursor-pointer">
        ...
        <div x-data="{ state: false, toggle: 'form', switchView() { this.toggle = (this.toggle === 'form' ? 'inventory' : 'form') } }" @modal.window="state = !state" @keydown.window.escape="state = false">
            <div x-show="state" class="fixed inset-0 bg-black/30 backdrop-blur-sm z-40 flex items-center justify-center"
                ...
                <div x-show="state" @click.outside="state = false"
                    ...
                    <div class="flex items-center justify-between p-5 border-b rounded-t border-gray-200">
                        <div class="flex space-x-4 items-center">
                            ...
                            <div @click="switchView()"
                                class="px-3 py-1 border-2 border-primary hover:bg-primary hover:text-white rounded-md cursor-pointer flex items-center gap-2 transition-all"
                                :class="toggle === 'inventory' ? 'bg-primary text-white' : ''">
                    ...
                    <!-- Form Input -->
                    <form x-show="toggle === 'form'" x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
                        wire:submit.prevent='save' class="p-5">
                        ...
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                ...
                                <input wire:model.live="name" type="text" id="name"
                                    class="bg-gray-50 border focus:outline-none focus:ring-primary transition-all text-gray-900 text-sm rounded-lg block w-full p-2.5 {{ $errors->has('name') ? 'border-red-500' : 'border-gray-300' }}">
                                @error('name')
                            ...
                            <div class="col-span-2">
                                <label for="campus" class="block mb-2 text-sm font-medium text-gray-900">Lokasi
                                    Kampus</label>
                                <select wire:model.change='campus_id' id="campus"
                                    class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 {{ $errors->has('campus_id') ? 'border-red-500' : 'border-gray-300' }}">
                            ...
                            <div class="col-span-2">
                                <label for="building" class="block mb-2 text-sm font-medium text-gray-900">Lokasi
                                    Gedung</label>
                                <select wire:model.change='building_id' id="building"
                                    class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 {{ $errors->has('building_id') ? 'border-red-500' : 'border-gray-300' }}">
                            ...
                            <div class="col-span-2">
                                <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Lokasi
                                    Lantai</label>
                                <select wire:model='floor' id="category"
                                    class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 {{ $errors->has('floor') ? 'border-red-500' : 'border-gray-300' }}">
                            ...
                            <div class="col-span-2">
                                ...
                                <input wire:model="capacity" type="text" id="capacity"
                                    class="bg-gray-50 border focus:outline-none focus:ring-primary transition-all text-gray-900 text-sm rounded-lg block w-full p-2.5 {{ $errors->has('capacity') ? 'border-red-500' : 'border-gray-300' }} }}">
                                @error('capacity')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="col-span-2">
                                ...
                                <select wire:model='category'
                                    class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 {{ $errors->has('category') ? 'border-red-500' : 'border-gray-300' }}">
                                    <option disabled>Pilih Kategori</option>
                                    <option value="general">Umum</option>
                                    <option value="class">Kelas</option>
                                    <option value="office">Kantor</option>
                                    <option value="laboratory">Laboratorium</option>
                                    <option value="open_space">Ruang Terbuka</option>
                                    <option value="internal_unj">Internal UNJ</option>
                                </select>
                                @error('category')
                                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="flex justify-between min-w-full">
                                <div x-data>
                                    <div class="flex items-center mb-4">
                                        ...
                                        <label for="default-radio-1"
                                            class="select-none ms-2 text-sm font-medium text-heading">Disewakan</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input type="radio" value="0" name="rentable"
                                            wire:model.live='rentable' name="rentable"
                                            class="w-4 h-4 text-neutral-primary border-default-medium bg-neutral-secondary-medium rounded-full checked:border-brand focus:ring-2 focus:outline-none focus:ring-brand-subtle border border-default appearance-none">
                                        <label for="default-radio-2"
                                            class="select-none ms-2 text-sm font-medium text-heading">Tidak
                                            Disewakan</label>
                                    </div>
                                    @error('rentable')
                                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                                ...
                                <div x-data>
                                    <div class="flex items-center mb-4">
                                        <input checked type="radio" value="1" name="show"
                                            wire:model.live='show'
                                            class="w-4 h-4 text-neutral-primary border-default-medium bg-neutral-secondary-medium rounded-full checked:border-brand focus:ring-2 focus:outline-none focus:ring-brand-subtle border border-default appearance-none">
                                        <label for="default-radio-1"
                                            class="select-none ms-2 text-sm font-medium text-heading">Publik</label>
                                    </div>
                                    <div class="flex items-center">
                                        <input type="radio" value="0" name="show" wire:model.live='show'
                                            name="show"
                                            class="w-4 h-4 text-neutral-primary border-default-medium bg-neutral-secondary-medium rounded-full checked:border-brand focus:ring-2 focus:outline-none focus:ring-brand-subtle border border-default appearance-none">
                                        <label for="default-radio-2"
                                            class="select-none ms-2 text-sm font-medium text-heading">Privat</label>
                                    </div>
                                    @error('show')
                                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                                    @enderror
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea wire:model="description" id="description" rows="4"
                                    class="bg-gray-50 border focus:outline-none focus:ring-primary transition-all text-gray-900 text-sm rounded-lg block w-full p-2.5 {{ $errors->has('description') ? 'border-red-500' : 'border-gray-300' }}"></textarea>
                                @error('description')
                                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div @close-modal.window="state = false" class="flex justify-end mt-8">
                            <button type="submit" wire:loading.attr="disabled"
                                wire:target="images_path,documents_path,save"
                                class="px-4 py-2 text-gray-900 font-semibold border-2 border-primary rounded-lg cursor-pointer hover:bg-primary hover:text-white transition-all focus:outline-none focus:bg-primary focus:text-white">
                    ...
                    {{-- Inventory Input --}}
                    <div x-data="{ items: @entangle('inventory'), addItem() { this.items.push({ name: '', quantity: 1 }) } }" class="p-5" x-show="toggle === 'inventory'"
                        ...
                        <div class="flex flex-col space-y-2">
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <div class="flex space-x-4 mb-3 items-center">
                                    <input type="text" x-model="item.name" placeholder="Nama Barang"
                                        class="flex-1 p-2 border border-primary rounded-md focus:outline-none focus:ring-primary transition-all">

                                    <input type="number" min="1" x-model.number="item.quantity"
                                        placeholder="Kuantitas"
                                        class="w-24 p-2 border border-primary rounded-md text-center focus:outline-none focus:ring-primary transition-all">

                                    <button @click="items.splice(index, 1)"
                                        class="p-2 text-red-600 hover:text-red-800 transition duration-150 cursor-pointer"
                                        title="Remove Item" x-show="items.length > 1">
                        ...
                        <button @click="addItem()"
                            class="flex space-x-1 items-center px-4 py-2 bg-primary text-white font-semibold rounded-md hover:bg-unj-dark transition duration-150 cursor-pointer">
                ...
                <tbody x-data="{ state: true }" x-show="state" @pending-toggle.window="state = !$event.detail">
                    @foreach ($rooms as $room)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                ...
                                @if (in_array($room->id, $rejected_rooms))
                                    <div class="flex items-center gap-2">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="#F44336" class="size-5">
                            ...
                            <td class="px-6 py-4">
                                {{ $room->rentable ? 'Disewakan' : 'Tidak Disewakan' }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $room->show ? 'Publik' : 'Privat' }}
                            </td>
                            <td class="px-6 py-4">
                                <button wire:click='view({{ $room->id }})' type="button"
                                    class="transition-all cursor-pointer hover:text-primary hover:bg-primary-light rounded-xl p-2 mx-auto"
                                    data-tip="Gambar">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    ...
                                    @if ($room->admin_id === Auth::user()->role || Auth::user()->role === '1')
                                        <a href="{{ route('edit-ruang', $room->slug) }}" wire:navigate
                                            class="transition-all cursor-pointer rounded-xl p-2 mx-auto {{ in_array($room->id, $rejected_rooms) ? 'text-red-500 hover:bg-red-200 tooltip tooltip-error' : 'hover:text-yellow-900 hover:bg-yellow-200' }}"
                                            data-tip="Perubahan ditolak">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <button wire:click='deleteModal({{ $room->id }})'
                ...
                <tbody x-data="{ state: false }" x-show="state" @pending-toggle.window="state = $event.detail">
                    ...
                    @foreach ($updates as $update)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            <th scope="row"
                                ...
                                @else
                                    <div class="flex gap-2 items-center">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="2" stroke="#EAB308" class="size-5">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <td class="px-6 py-4">
                                {{ $update->new_data['length'] }} m<sup>2</sup>
                            </td>
                            <td class="px-6 py-4">
                                {{ $update->new_data['width'] }} m<sup>2</sup>
                            </td>
                            <td class="px-6 py-4">
                                {{ $update->new_data['height'] }} m<sup>2</sup>
                            </td>
                            <td class="px-6 py-4">
                                @if ($update->new_data['category'] === 'class')
                                    <span class="px-2 py-1 rounded-lg bg-[#007BFF] text-white">Kelas</span>
                                @elseif ($update->new_data['category'] === 'office')
                                    <span class="px-2 py-1 rounded-lg bg-[#17A2B8] text-white">Kantor</span>
                                @elseif ($update->new_data['category'] === 'laboratory')
                                    <span class="px-2 py-1 rounded-lg bg-[#6F42C1] text-white">Laboratorium</span>
                                @elseif ($update->new_data['category'] === 'open_space')
                                    <span class="px-2 py-1 rounded-lg bg-[#01a2ff] text-white">Ruang Terbuka</span>
                                @elseif ($update->new_data['category'] === 'internal_unj')
                                    <span class="px-2 py-1 rounded-lg bg-[#006569] text-white">Internal UNJ</span>
                                @elseif ($update->new_data['category'] === 'general')
                                    <span class="px-2 py-1 rounded-lg bg-[#28A745] text-white">Umum</span>
                                @endif
                            </td>

                            <td class="px-6 py-4">
                                {{ $update->new_data['rentable'] ? 'Disewakan' : 'Tidak Disewakan' }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $update->new_data['show'] ? 'Publik' : 'Privat' }}
                            </td>
                            <td class="px-6 py-4">
                                <button wire:click='viewPending({{ $update->id }})' type="button"
                                    class="transition-all cursor-pointer hover:text-primary hover:bg-primary-light rounded-xl p-2 mx-auto"
                                    data-tip="Gambar">
                                    <i class="fa-solid fa-images"></i>
                                </button>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex gap-2 items-center">
                                    ...
                                    @if ($update->admin_id === Auth::user()->role || Auth::user()->role === '1')
                                        <a href="{{ route('edit-ruang', $update->id) }}" wire:navigate
                                            class="transition-all cursor-pointer rounded-xl p-2 mx-auto {{ $update->status == 'rejected' ? 'text-red-500 hover:bg-red-200 tooltip tooltip-error' : 'hover:text-yellow-900 hover:bg-yellow-200' }}"
                                            data-tip="Perubahan ditolak">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                    @endif
    ...
    <div x-data="{ state: false, 'status': '', 'message': '' }" x-show="state"
        ...
        {{-- Failed Logo --}}
        <div :class="status === 'fail' ? 'inline-flex' : 'hidden'"
                ...
                <div class="flex flex-col items-center py-8">
                    ...
                    <div class="flex gap-6">
                        <button wire:click='deleteRoom'
                            class="px-8 py-2 rounded-md bg-primary hover:bg-unj-dark transition-all cursor-pointer text-white text-xl">Iya</button>
                        <button @click="$dispatch('confirm-delete')"
                            class="px-8 py-2 rounded-md border-2 border-red-600  hover:bg-red-700 hover:border-red-700 transition-all cursor-pointer text-xl hover:text-white">Tidak</button>
                    </div>
                </div>
            </div>
        </div>
