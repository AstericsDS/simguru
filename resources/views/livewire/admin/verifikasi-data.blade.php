                    ...
                    <td>
                        <div class="flex gap-2 justify-center">
                            ...
                            @if ($data->status == 'pending')
                                <button data-modal-target="popup-accept-{{ $data->id }}" data-modal-toggle="popup-accept-{{ $data->id }}" class="p-2 bg-green-200 text-green-500 rounded-md cursor-pointer font-semibold hover:bg-green-300 transition-all" type="button">
                                    Setuju
                                </button>

                                <div id="popup-accept-{{ $data->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                                            <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white transition-all cursor-pointer" data-modal-hide="popup-accept-{{$data->id}}">
                                            ...
                                            <div class="p-4 md:p-5 text-center">
                                                ...
                                                <button wire:click='accept({{ $data->id }})' data-modal-hide="popup-accept-{{ $data->id }}" type="button" class="text-white bg-green-600 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center cursor-pointer transition-all">
                                                    Iya
                                                </button>
                                                <button data-modal-hide="popup-accept-{{ $data->id }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white transition-all cursor-pointer dark:hover:bg-gray-700">Batal</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <button data-modal-target="popup-reject-{{ $data->id }}" data-modal-toggle="popup-reject-{{ $data->id }}" class="p-2 bg-red-200 text-red-500 rounded-md cursor-pointer font-semibold hover:bg-red-300 transition-all" type="button">
                                    Tolak
                                </button>

                                <div id="popup-reject-{{$data->id}}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                                            <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white transition-all cursor-pointer" data-modal-hide="popup-reject-{{$data->id}}">
                                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                                </svg>
                                                <span class="sr-only">Close modal</span>
                                            ...
                                            <div class="p-4 md:p-5 text-center">
                                                ...
                                                <button wire:click='reject({{ $data->id }})' data-modal-hide="popup-reject-{{ $data->id }}" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center cursor-pointer transition-all">
                                                    Iya
                                                </button>
                                                <button data-modal-hide="popup-reject-{{ $data->id }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 cursor-pointer transition-all">Batal</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
