<div>
                ...
                <tbody>
                    @foreach ($events as $event)
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            ...
                            <td class="px-6 py-4">
                                <span>
                                    {{ $event->room->building->name }}, {{ $event->room->campus->name }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span>
                                    {{ $s->translatedFormat('H:i') }} - {{ $e->translatedFormat('H:i') }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 flex">
                                @if ($event->verified === 'pending')
                                    <span class="px-2 py-1 border border-yellow-500 bg-yellow-200 rounded-md flex-1 flex justify-center">Pending</span>
                                @elseif ($event->verified === 'approved')
                                    <span class="px-2 py-1 border border-green-500 bg-green-200 rounded-md flex-1 flex justify-center">Disetujui</span>
                                @else
                                    <span class="px-2 py-1 border border-red-500 bg-red-200 rounded-md flex-1 flex justify-center">Ditolak</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2 items-center">
                                    @if ($event->verified === 'pending')
                                        <button @click="$dispatch('confirm-modal', {id: {{ $event->id }}, action: 'accept'})" class="border border-green-500 size-8 rounded-xl cursor-pointer hover:bg-green-500 hover:text-white transition-all">
                                            <i class="fa-solid fa-check"></i>
                                        </button>
                                        <button @click="$dispatch('confirm-modal', {id: {{$event->id}}, action: 'reject'})" class="border border-red-500 size-8 rounded-xl cursor-pointer hover:bg-red-500 hover:text-white transition-all">
    ...
    {{-- Confirm Modal --}}
    <div x-data="{ state: false, confirmData: { id: null, action: null } }" @confirm-modal.window="state = !state; confirmData = $event.detail" @keydown.window.escape="state = false">
            ...
            <div x-show="state" @click.outside="state = false" class="relative bg-white max-h-screen overflow-y-auto rounded-lg shadow-sm w-2xl p-2 opacity-100 z-50" x-transition:enter="transition ease-in-out duration-250" x-transition:enter-start="scale-50" x-transition:enter-end="scale-100" x-transition:leave="transition ease-in-out duration-250" x-transition:leave-end="scale-50">
                ...
                <div class="flex flex-col items-center py-8">
                    ...
                    <div class="flex gap-6">
                        <button @click="$wire.save(confirmData.id, confirmData.action)" class="px-8 py-2 rounded-md bg-primary hover:bg-primary-dark transition-all cursor-pointer text-white text-xl">Iya</button>
