<div>
  ...
  {{-- Detail Kampus --}}
  <div class="flex gap-8 xl:flex-row flex-col mt-8">
    @if ($campus?->images_path || $pending?->new_data["images_path"])
      {{-- Media --}}
      <div class="flex flex-col gap-4 xl:flex-3">
        {{-- Slider --}}
        <div class="swiper w-full xl:w-fit relative rounded-md overflow-hidden">
          <div class="swiper-wrapper">
            @foreach ($campus?->images_path ?? ($pending?->new_data["images_path"] ?? []) as $index => $image)
              <div class="swiper-slide" wire:key="swiper-slide-{{ $index }}">
