<div class="w-full p-5 lg:px-60">
  ...
  <div class="flex flex-col my-5">
    ...
    <div class="breadcrumbs text-sm text-[#006569]">
      <ul>
        <li>
          <a href="/kampus/{{ $building->campus->slug }}">
            {{ $building->campus->name }}
          </a>
        </li>
        <li>
          <a href="/gedung/{{ $building->slug }}">Gedung Dewi Sartika</a>
        </li>
      </ul>
    </div>
  </div>
    ...
    <div class="row-span-3 not-lg:hidden">
      ...
      <div class="grid grid-flow-row gap-3 font-semibold">
        <div class="grid grid-cols-[150px_10px_auto]">
          ...
          <div>:</div>
          <div>
            {{ $building->campus->name ?? "Pengguna Tidak Ditemukan" }}
          </div>
        </div>
        <div class="grid grid-cols-[150px_10px_auto]">
          <div>Alamat</div>
              ...
              <div class="card card-sm bg-primary shadow-xl h-max">
                <div class="flex items-center justify-between p-3">
                  <div class="flex flex-wrap gap-2">
                    <div>{{ $room->name }}</div>
                    <div
                      class="badge badge-sm badge-soft badge-success self-center"
                    >
                      Tersedia
                    </div>
                  </div>
