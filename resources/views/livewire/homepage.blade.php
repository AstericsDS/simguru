<div id="body-ko" class="flex flex-col min-w-full">
  ...
  >
    ...
    <h1
      class="text-xl lg:text-5xl text-white text-wrap w-3/7 text-center font-semibold"
    >
    ...
    <a
      ...
      class="btn btn-xl bg-[#FDDC00] text-primary border-none shadow-none flex items-center gap-2"
    >
      <svg
  ...
  <div class="hero bg-white text-black min-h-screen">
    <div class="hero-content flex-col lg:flex-row">
      <img
        src="{{ asset("backgrounds/unj_bersih.jpeg") }}"
        class="max-w-xs lg:max-w-2xl rounded-lg shadow-2xl"
      />
      <div class="max-w-xl">
        ...
        <p class="py-6 text-justify">
          Sistem Informasi Gedung dan Ruang adalah platform yang menyediakan
          informasi lengkap mengenai gedung dan ruang di Universitas Negeri
          Jakarta.
        </p>
  ...
  <h5
    id="alert"
    class="hidden alert text-base-content bg-base-200 font-bold text-center"
  >
    Gunakan Landscape Untuk Melihat Jadwal Kelas!
  </h5>

  {{-- judul container --}}
  <div class="text-black text-center flex flex-col gap-3 pt-20" id="kampus">
    <h1 class="text-2xl lg:text-4xl font-semibold">
      Universitas Negeri Jakarta Multi Kampus
    </h1>
    <hr class="w-15 font-bold mx-auto border-gray-500 border" />
    <p class="text-gray-500">Kampus-Kampus UNJ</p>
  </div>
    ...
    @foreach ($campuses as $campus)
      ...
      >
        <figure class="max-h-60 self-center">
          <img
            src="{{ isset($campus->images_path) ? asset("storage/" . $campus->images_path[0]) : asset("backgrounds/DUMMY.png") }}"
            alt="Kampus_A_UNJ"
          />
        </figure>
        <div class="card-body items-center text-center">
          ...
          <div class="card-actions">
            <a
              href="/kampus/{{ $campus->slug }}"
              class="btn bg-white text-black w-full hover:bg-gray-200 rounded-lg outline-none"
            >
              Details
            </a>
          </div>
  ...
  <button
    class="btn border-none shadow-none mt-5 self-center rounded-full bg-primary"
  >
    <a href="/kampus">LIHAT KAMPUS</a>
  </button>
  {{-- list beberapa gedung di UNJ --}}
  <div class="text-black text-center flex flex-col gap-3 pt-20" id="gedung">
    <h1 class="text-2xl lg:text-4xl font-semibold">
      Gedung - Gedung Universitas Negeri Jakarta
    </h1>
    <hr class="w-15 font-bold mx-auto border-gray-500 border" />
  </div>
  ...
  <div class="flex flex-wrap justify-center mt-3 gap-3 lg:px-50">
    @foreach ($buildings as $building)
      ...
      >
        <figure class="max-h-60 self-center">
          <img
            src="{{ isset($building->images_path) ? asset("storage/" . $building->images_path[0]) : asset("backgrounds/DUMMY.png") }}"
            alt="{{ $building->name }}"
          />
        </figure>
        <div class="card-body items-center text-center">
          <h2 class="card-title">{{ $building->name }}</h2>
          <p class="text-xs not-lg:hidden">
            {{ $building->address }}
          </p>
          <div class="card-actions">
            <a
              href="/gedung/{{ $building->slug }}"
              class="btn bg-white text-black w-full hover:bg-gray-200 rounded-lg outline-none"
            >
              Details
            </a>
  ...
  <button
    class="btn border-none shadow-none mt-5 self-center rounded-full bg-primary"
  >
    <a href="/gedung">LIHAT GEDUNG</a>
  </button>
  {{-- Ruangan container --}}
  <div class="text-black text-center flex flex-col gap-3 pt-20" id="ruang">
    <h1 class="text-2xl lg:text-4xl font-semibold" id="ruang">
      Ruangan Universitas Negeri Jakarta
    </h1>
    <hr class="w-15 font-bold mx-auto border-gray-500 border" />
  </div>
  <div class="flex flex-wrap justify-center mt-3 gap-3 lg:mx-50">
    @foreach ($rooms as $room)
      ...
      >
        <figure class="max-h-60 self-center">
          <img
            src="{{ isset($room->images_path) ? asset("storage/" . $room->images_path[0]) : asset("backgrounds/DUMMY.png") }}"
            alt="Kampus_A_UNJ"
          />
        </figure>
        <div class="card-body">
  ...
  <button
    class="btn border-none shadow-none mt-5 self-center rounded-full bg-primary"
  >
    <a href="/ruang">LIHAT RUANG</a>
  </button>
  <div class="text-black text-center flex flex-col gap-3 pt-20" id="statistik">
    <h1 class="text-4xl font-semibold">
      Grafik Statistik Informasi Manajemen Gedung dan Ruang
    </h1>
    <hr class="w-15 font-bold mx-auto border-gray-500 border" />
    <p class="text-gray-500">
      Berikut adalah Grafik Statistik Informasi Manajemen Gedung dan Ruang
